...
const {search ,setSearch ,getTotalCartItems_} = useContext(ShopContext)
            ...
            navs.map((nav) => (
              <li  key={nav.title}>
                <Link  className={`nav_links 
                    focus:border-b border-gray-500
...
<Link className=" py-2 hover:text-[#e68c82] transition-all text-gray-600 tracking-wide outfit " href="/orders" >
orders
</Link>
<Link className=" py-2 hover:text-[#e68c82] transition-all text-gray-600 tracking-wide outfit " href="/signout" >
Signout
</Link>
</div>
...
<span className="absolute size-4 text-[9px] text-center flex justify-center items-center p-1 text-white rounded-full top-0 left-3 bg-black " >
...
<div className="ml-2 mg:hidden cursor-pointer  " onClick={()=>setShowMobileMenu(true)} >
  <Image src={assets.menu_icon} alt="menu  icon" className="h-4 w-7 " />
</div>
